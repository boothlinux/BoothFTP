#!/usr/bin/env python3
#
#    Booth FTP, copyright Colton Booth
#
...
import BoothFTP_ToolbarButton as ToolbarButton
import BoothFTP_FileDialogs as Filedialogs
import platform
...
#Change the left hand menu to say Booth FTP
...
class app:
    def __init__(self, master):
        ...
        #An index that points to current file that the mouse== pointing
        ...
        #Variable to store which cell cursor== currently pointing
        self.x_cell_pos = 0
        self.y_cell_pos = 0
        ...
        self.BoothFTP_icon = PhotoImage(file='Icons/BoothFTP_large.png')
        self.goto_icon = PhotoImage(file='Icons/gotopath_big.png')

        #Load glow version of icons
        self.connect_glow_icon = PhotoImage(file='Icons/connect_big.png')
        self.upload_glow_icon = PhotoImage(file='Icons/upload_big.png')
        self.download_glow_icon = PhotoImage(file='Icons/download_big.png')
        self.newfolder_glow_icon = PhotoImage(file='Icons/newfolder_big.png')
        self.up_glow_icon = PhotoImage(file='Icons/up_big.png')
        self.info_glow_icon = PhotoImage(file='Icons/info_big.png')
        self.delete_glow_icon = PhotoImage(file='Icons/delete_big.png')
        self.properties_glow_icon = PhotoImage(file='Icons/properties_big.png')
        self.cut_glow_icon = PhotoImage(file='Icons/cut_big.png')
        self.copy_glow_icon = PhotoImage(file='Icons/copy_big.png')
        self.paste_glow_icon = PhotoImage(file='Icons/paste_big.png')
        self.console_glow_icon = PhotoImage(file='Icons/console_big.png')
        self.search_glow_icon = PhotoImage(file='Icons/search_big.png')
        self.BoothFTP_glow_icon = PhotoImage(file='Icons/BoothFTP_large.png')
        self.dnd_glow_icon = PhotoImage(file='Icons/upload_big.png')
        self.goto_glow_icon = PhotoImage(file='Icons/gotopath_big.png')
        ...
        #Set window icon
        self.master.iconphoto(True, self.BoothFTP_icon)
    ...
    def update_file_list_thread(self):
        try:
            ...
            thread_request_queue.put(lambda:self.master.wm_title('BoothFTP - '+self.ftpController.pwd()))
            thread_request_queue.put(lambda:self.unlock_status_bar())
    ...
    def draw_icons(self, event = None):    
        ...
        if len(self.file_list)== 0:
            self.canvas.create_image(self.canvas_width/2, self.canvas_height/2, image = self.BoothFTP_glow_icon)
    ...
    def update_status(self, event = None, message = ' '):
        ...
        self.canvas.itemconfig(self.rect_id, outline = '')
        self.canvas.coords(self.rect_id, -1, -1, -1, -1) 
        #Display message in status bar in black color only if change_status== true else ignore it
    ...
    def update_status_red(self, message):
        ...
        self.canvas.itemconfig(self.rect_id, outline = '')
        self.canvas.coords(self.rect_id, -1, -1, -1, -1) 
        #Display message in status bar in red color only if change_status== true else ignore it
        if self.change_status== True:
    ...
    def upload_window(self):
        self.upload_dialog = Filedialogs.open_file_dialog(self.master, 'Choose files or folders to upload', self.upload_thread)
    ...
    def download_window(self):
        #Check number of files selected
        if(len(self.selected_file_indices) < 1): return
        self.download_dialog = Filedialogs.open_file_dialog(self.master, 'Choose folder, or drag n drop here', self.download_thread, True)
    ...
    def clipboard_cut(self, event = None):
        ...
        for index in self.selected_file_indices:
            #If it== a search result get the clipboard path from the search result
    ...
    def clipboard_paste(self, ftpController, clipboard_path_list, clipboard_file_list, detailed_clipboard_file_list, cut, copy):        
        #Set current status
        thread_request_queue.put(lambda:self.update_status('Moving...'))        
        if(cut== True):
            #Loop through all selected files and folders
            ...
            #Set current status
            thread_request_queue.put(lambda:self.update_status('Copying...'))
            #Loop through all selected files and folders
            for clipboard_path, file_name, file_details in zip(clipboard_path_list, clipboard_file_list, detailed_clipboard_file_list):
                ...
                try:
                    ...
                    thread_request_queue.put(lambda:self.progress('Failed to copy!', file_name))
            thread_request_queue.put(lambda:self.clear_clipboard())
            thread_request_queue.put(lambda:self.progress('You can now close the window', 'Done'))
        #update file list and redraw icons
        thread_request_queue.put(lambda:self.update_file_list())
    ...
    def ask_replace(self, file_name, status):
        ...
        self.replace_window = Filedialogs.replace_dialog(self.console_window.console_dialog_window, 'Duplicate files!', self.copy_icon, file_name+': '+status+', Replace?')
        #Loop till a button== pressed
    ...
    def info(self):
        self.info_window = Filedialogs.about_dialog(self.master, 'About', self.BoothFTP_icon, 'BoothFTP v0.1', '© Colton Booth') 
    ...
    def do_wait(self, event = None):
        ...
        #make sure frame index== not above 4
